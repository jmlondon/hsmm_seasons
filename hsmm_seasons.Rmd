...
Between 2009 and 2012 seven adult bearded seals were captured and released with bio-logging devices that provide measures of movement, dive behavior, and hourly wet-dry proportions. The tags were deployed in June and July with deployment duration ranging from `r min(deployments$duration)` to `r max(deployments$duration)` days (median = `r median(deployments$duration)` days). Data from these deployments are available as an R data package and we will use these data to demonstrate the application of multivariate hidden semi-Markov models to identify seasonal states from real world telemetry data.
...
The R package `mhsmm` provides a framework for multi-variate hidden semi-Markov models. The package ships with support for selected emmission distributions (Poisson, normal, and multi-variate normal). For dive behavior (i.e., number of dives below 10m), a Poisson emmission distribution was used and a multi-variate normal was used for the combined x-y displacement. A Bernouli distribution was used for haul-out status. Custom M-step functions were written to extend the package in support of the Bernouli as well as provide support for missing values in the Poisson and multi-variate normal.
...
In order to mimic the seasonal progression, we fixed the transition matrix with all superdiagonal entries set to 1 and the remaining entries set to 0. The transition matrix will not be re-estimated and the initial state was set to state 1. Initial values for the other parameters were generally set to a reasonable value and repeated across all states (table 1).
...
init_vals <- data.frame(
  state = c(1:n_states),
  init_state = c(1, rep(0, n_states - 1)),
  haul_out = rep(0.05,n_states),
  number_dives = rep(30, n_states),
  xy_disp = rep("0,0",n_states),
  sojourn_type = rep("poisson",n_states),
  sojourn_lambda = c("60", rep(paste0("nrows/",n_states - 1),
                               n_states - 1)),
  sojourn_shift = rep(0,n_states),
...
mshmm_telem_fit <- function(dd, J) {
  ...
  loglik <- tail(fit$loglik, n = 1)
  
  fit_AIC = -2 * loglik + 2 * npar
...
for (id in unique(dd$deployid)) {
    this_data <- dd %>% filter(deployid == id) %>% 
    arrange(deployid, date_time)
    message(id)
  for (J in 3:8) {
...
for (id in unique(dd$deployid)) {
  ...
  fit_data[[id]]$state <- state
}
dd_fit <- bind_cols(dd,bind_rows(fit_data))
dd_fit$state = as.factor(dd_fit$state)
