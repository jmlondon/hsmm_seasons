...
In order to mimic the seasonal progression, we fixed the transition matrix with all superdiagonal entries set to 1 and the remaining entries set to 0. The transition matrix was not be re-estimated and the initial state was set to state 1. Initial values for the other parameters were generally set to a reasonable value and repeated across all states (table 1). One key initial value that diverged from this approach was the sojourn distribution. A shifted Poisson distribution was chosen and the lambda value was set to the number of time steps divided by the number of time steps. This initiates the model with a constant sojourn value distributed evenly across the deployment. However, the adult bearded seals were captured during the later stages of their annual molt. Given the significant behavioral changes associated with molt, the first state is likely to be of a relatively short duration. Thus, we specified a lambda of 40 for the first state regardless of the number of states. 
...
init_vals <- data.frame(
  ...
  sojourn_lambda = c("40", rep(paste0("nrows/",n_states - 1),
                               n_states - 1)),
