...
predData %>% mutate(direction = ifelse(
...
for (id in unique(dd$deployid)) {
  ...
  fit_data[[id]]$state <- predict(this_fit,this_data)$s
}
...
library(ggplot2)
ggplot(data=dd_fit,aes(x=days_since_apr1,y="")) + 
...
ggplot(data=combo,aes(x=days_since_apr1,y=" ")) + 
  ...
  scale_fill_discrete(limits=levels(dd_fit$state),drop=TRUE) +
...
```{r plot-9, echo=FALSE}
ggplot(data=dd_fit %>% filter(percent_dry>3),aes(x=percent_dry)) +
...
ggplot(data=dd_fit %>% filter(num_dives<150),aes(x=num_dives)) +
  geom_density(aes(fill=state),size=0) + facet_wrap(~state) +
