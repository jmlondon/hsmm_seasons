...
p1 <- ggplot(predData, aes(x=deploy_day, y=ho_binary)) + 
  geom_point(alpha=0.2) + facet_grid(deployid ~ .) + 
  ggtitle("haul-out status") +
  usweb_theme()
p1
```
...
p2 <- ggplot(predData, aes(x=deploy_day, y=num_dives)) + 
  geom_point(alpha=0.2) + facet_grid(deployid ~ .) + 
  ggtitle("number of dives below 10m") +
  usweb_theme()
p2
```
...
ggplot(predData,aes(x=deploy_day, y=x_disp/1000)) +
  geom_bar(aes(color=factor(direction)),alpha=0.2,stat="identity") +
  facet_grid(deployid ~ .) + 
  ggtitle("x displacement") +
  usweb_theme()
```
...
library(ggplot2)
ggplot(data=predData,aes(x=deploy_day,y="")) + 
...
ggplot(data=combo,aes(x=deploy_day,y=" ")) + 
  geom_tile(aes(fill=state,alpha=counter/7)) +
  scale_fill_discrete(limits=levels(predData$state),drop=TRUE) +
  scale_alpha(guide = 'none') + usweb_theme()+
   theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank()) +
  ggtitle("Combined Seasonal State Assignments Across 7 Adult Bearded Seals")
```
...
```{r plot-7, echo=FALSE}
ggplot(data=predData %>% filter(percent_dry>3),aes(x=percent_dry)) +
...
```{r plot-8, echo=FALSE}
ggplot(data=predData %>% filter(num_dives<150),aes(x=num_dives)) +
  geom_density(aes(fill=state),size=0) + facet_wrap(~state) +
