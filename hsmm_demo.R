...
# Predict states
predData$state = predict(fit, data,method="smoothed")$s

library(ggplot2)
ggplot(data=predData,aes(x=deploy_day,y="state")) + geom_tile(aes(fill=as.factor(state)) + facet_wrap(~deployid, ncol=1)
